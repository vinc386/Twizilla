...
describe Micropost do
  ...
  describe "from_user_followed_by" do
    before(:each) do
      ...
      @other_post = @other_user.microposts.create!(:content => "foobar".reverse)
      @another_post = @another_user.microposts.create!(:content => "foobar".upcase)
      
      @user.follow!(@other_user)
    end
    
    it "should have a from_user_followed_by" do
    ...
    it "should include users's own microposts" do
      Micropost.from_users_followed_by(@user).
        should include(@user_post)
    end
    
    it "should not include non-followed users posts " do
      Micropost.from_users_followed_by(@user).
        should_not include(@another_post)
    end
  end
